...
const columnsObject: { [key: string]: (field: FieldFragment, model?: ModelFragment | null) => Column } = {
  ...
  object: (field) => ({
    ...
    Cell: ({ value }) => {
      ...
      return (
        <Button
          onClick={() => router.push(`/admin/models/${field.type}?${model.idField}=${value[model.idField]}`)}
          appearance="ghost"
          size="Small"
          fullWidth
          style={{
  ...
  list: (field, model) => ({
    ...
    Cell: ({
      ...
      if (!model) return <></>;
      return (
        <Button onClick={() => router.push(`/admin/models/${field.type}?${model.id}=${id}`)} appearance="ghost" size="Small">
          Show
        </Button>
      );
    },
